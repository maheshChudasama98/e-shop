...
import CertificatesModifyModal from './ColorModifyModal';
import { ColorDeleteService, ColorsListService } from 'src/Services/master.services';
...
export default function Index() {
  ...
  const DeleteActions = (id) => {
    sweetAlertQuestion(
      'This color will be permanently deleted. You won’t be able to recover it.',
      'Delete Color?'
    )
      .then((result) => {
        if (result === 'Yes') {
          dispatch(
            ColorDeleteService({ color_id: id }, (res) => {
              if (res?.status) {
                setApiFlag(!apiFlag);
                sweetAlertSuccess('Color deleted successfully').then(() => {});
              } else {

import {
  ...
  SERVER_ERROR_CODE,
  //   UNAUTHORIZED_CODE,
  TOKEN_NOT_VALID_CODE,
  TOO_MANY_REQUESTS_CODE,
  TOKEN_NOT_PROVIDED_CODE,
} from 'src/constance';
...
jwtAuthAxios.interceptors.response.use(
  ...
  async (err) => {
    if (
      err?.response?.status === BAD_REQUEST_CODE ||
      err?.response?.status === ERROR_CODE ||
      err?.response?.status === SERVER_ERROR_CODE
    ) {
      return Promise.reject(err?.response?.data);
    }

    if (
      err?.response?.status === TOKEN_NOT_VALID_CODE ||
      err?.response?.status === TOKEN_NOT_PROVIDED_CODE
    ) {
      localStorage.removeItem('token');
      window.location.replace('/login');

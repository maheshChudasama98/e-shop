...
import TabModifyModal from './TabModifyModal';
import { TabDeleteService, TabsListService } from 'src/Services/master.services';
...
export default function Index() {
  ...
  const DeleteActions = (id) => {
    sweetAlertQuestion(
      'This tab will be permanently deleted. You won’t be able to recover it.',
      'Delete Tab?'
    )
      .then((result) => {
        if (result === 'Yes') {
          dispatch(
            TabDeleteService({ tab_id: id }, (res) => {
              if (res?.status) {
                setApiFlag(!apiFlag);
                sweetAlertSuccess('Tab deleted successfully').then(() => {});
              } else {

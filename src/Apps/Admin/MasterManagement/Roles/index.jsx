...
import RoleModifyModal from './RoleModifyModal';
import { RolesDeleteService, RolesListService } from 'src/Services/master.services';
...
export default function Index() {
  ...
  const DeleteActions = (id) => {
    sweetAlertQuestion(
      'This role will be permanently deleted. You won’t be able to recover it.',
      'Delete Role?'
    )
      .then((result) => {
        if (result === 'Yes') {
          dispatch(
            RolesDeleteService({ role_id: id }, (res) => {
              if (res?.status) {
                setApiFlag(!apiFlag);
                sweetAlertSuccess('Role deleted successfully').then(() => {});
              } else {

...
export default function Index() {
  ...
  const DeleteActions = (id) => {
    sweetAlertQuestion(
      'This category will be permanently deleted. You won’t be able to recover it.',
      'Delete Category?'
    )
      .then((result) => {
        if (result === 'Yes') {
          dispatch(
            CategoryDeleteService({ category_id: id }, (res) => {
              // setApiFlag(!apiFlag);
              if (res?.status) {
                setApiFlag(!apiFlag);
                sweetAlertSuccess('Category deleted successfully').then(() => {});
              } else {
  ...
  useEffect(() => {
    ...
    dispatch(
      CategoriesListService(payLoad, (res) => {
        if (res?.status) {

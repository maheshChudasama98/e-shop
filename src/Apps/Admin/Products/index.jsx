...
export default function Index() {
  ...
  useEffect(() => {
    ...
    dispatch(
      ProductsListService(payLoad, (res) => {
        if (res?.status) {
  ...
  useEffect(() => {
    dispatch(
      SelectorsService({ categories: true, products_status: true, brands: true }, (res) => {
        if (res?.status) {
          ...
          <>
            ...
            >
              <Table
                ...
                onRow={(record) => ({
                  onClick: () => {
                    router.push(`${ADMIN_ROUTES.PRODUCT_DETAILS}?id=${record.product_id}`);
                  },
